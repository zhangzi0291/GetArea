...
def getNameAndUrl(ele):
  if len(ele.find("td")) == 2:
    ...
    if( len(ele.find('td:nth-child(1)>a')) >0 ):
      return ele.find('td:nth-child(3)>a')[0].text,ele.find('td:nth-child(1)>a')[0].text,ele.find('td:nth-child(2)>a')[0].text
    else:
      return ele.find('td:nth-child(3)')[0].text,ele.find('td:nth-child(1)')[0].text,ele.find('td:nth-child(2)')[0].text
...
response = session.get('http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2020/index.html')
response.encoding = 'GBK'
provincetrList = response.html.find('.provincetr>td>a')
print("[")
for provincetr in provincetrList:
  provinceurl = list(provincetr.links)[0];
  province = {'name':provincetr.text,'url':baseurl+provinceurl,'code':provinceurl.replace('.html',''),'pcode':'-1','region_level':1}
  print(province,",")
